...
fun AppNavigation() {
    ...
    NavHost(
        ...
        composable("nurse_command_center") {
            NurseCommandCenterScreen(onBack = { navController.popBackStack() }, onNavigate = { route -> navController.navigate(route) })
        }
        composable("care_pulse") {
            CarePulseScreen(onNavigate = { route -> navController.navigate(route) }, onBack = { navController.popBackStack() })
        }
        composable("profile") {
            ProfileScreen(viewModel = viewModel, onNavigateToClaimPeriod = { _, _ -> navController.navigate("claim_period") { popUpTo("home") { inclusive = false } } })
        }
        composable("claim_period") {
            ClaimPeriodScreen(
                onNavigateToDailyEntry = { claimPeriodId, start, end, wardType -> navController.navigate("daily_entry/$claimPeriodId/$start/$end/$wardType") },
        ...
        composable("advanced_finance_hub") {
            ...
            AdvancedFinanceHubScreen(viewModel = advancedFinanceViewModel, onNavigate = { route -> navController.navigate(route) }, onBack = { navController.popBackStack() })
        }
        ...
        composable(
            ...
            DailyEntryScreen(
                ...
                onSaveAndSharePdf = { file ->
                    ...
                    if (shareUri != null) { FileShareUtils.showSavedToast(context); shareSavedPdf(context, shareUri) }
                }
            )
        }
    }
