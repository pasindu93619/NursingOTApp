    ...
    Scaffold(
        ...
        Column(
            ...
            AnimatedVisibility(visible = showHistory, enter = fadeIn(tween(250)) + slideInVertically(tween(300)) { it / 2 }, exit = fadeOut(tween(200)) + slideOutVertically(tween(200)) { it / 2 }) {
                MonthlyRecordPreview(basicSalary = basic, otHours = ot, dutyHours = duty, phHours = ph, gross = grossBeforeDeductions, net = estimatedNet, deductions = totalDeductions, workingDays = days)
            }
...
private fun SectionHeader(eyebrow: String, title: String, subtitle: String) {
    Column(modifier = Modifier.padding(horizontal = 2.dp)) {
        Text(eyebrow, fontSize = 10.sp, fontWeight = FontWeight.Black, color = Indigo)
...
private fun HeroEarningsCard(net: Double, gross: Double, otEarnings: Double, otTrend: Double) {
    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(28.dp), colors = CardDefaults.cardColors(containerColor = Indigo)) {
        Column(modifier = Modifier.padding(20.dp)) {
            Text("ESTIMATED NET", color = Color.White.copy(alpha = 0.75f), fontSize = 10.sp, fontWeight = FontWeight.Black)
            Text(formatRs(net), color = Color.White, fontSize = 30.sp, fontWeight = FontWeight.Black)
            Spacer(Modifier.height(14.dp))
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                HeroMetric("GROSS", gross, Cyan, Modifier.weight(1f))
                HeroMetric("OT", otEarnings, Orange, Modifier.weight(1f))
            }
            Spacer(Modifier.height(8.dp))
            Text("OT trend ${if (otTrend >= 0) "+" else ""}${otTrend.oneDecimal()}%", color = Color.White.copy(alpha = 0.75f), fontSize = 10.sp)
        }
    }
}

@Composable
private fun HeroMetric(label: String, value: Double, accent: Color, modifier: Modifier) {
    Surface(modifier = modifier, color = Color.White.copy(alpha = 0.10f), shape = RoundedCornerShape(16.dp)) {
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            Text(formatRs(value), color = Color.White, fontSize = 16.sp, fontWeight = FontWeight.Bold)
        }
    }
}
...
private fun SalaryMakerCard(expanded: Boolean, onExpand: () -> Unit, basicSalary: String, onBasicSalaryChange: (String) -> Unit, otRate: String, onOtRateChange: (String) -> Unit, otHours: String, onOtHoursChange: (String) -> Unit, phHours: String, onPhHoursChange: (String) -> Unit, dutyHours: String, onDutyHoursChange: (String) -> Unit, workingDays: String, onWorkingDaysChange: (String) -> Unit, otherDeduction: String, onOtherDeductionChange: (String) -> Unit, phRate: Double, otEarnings: Double, phEarnings: Double, gross: Double, tax: Double, wop: Double, otherDeductionAmount: Double, net: Double) {
    Card(modifier = Modifier.fillMaxWidth().animateContentSize(), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Column(modifier = Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Surface(color = Indigo.copy(alpha = 0.10f), shape = RoundedCornerShape(14.dp)) { Icon(Icons.Default.Payments, contentDescription = null, tint = Indigo, modifier = Modifier.padding(9.dp)) }
                    Spacer(Modifier.width(10.dp))
                    Column { Text("Monthly Salary Maker", fontSize = 16.sp, fontWeight = FontWeight.Bold, color = Ink); Text("Inputs → estimate", fontSize = 11.sp, color = Slate) }
                }
                IconButton(onClick = onExpand) { Icon(Icons.Default.ChevronRight, contentDescription = "Expand", tint = Slate) }
            }
            AnimatedVisibility(visible = expanded) {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    MetricRow("Estimated net", formatRs(net))
                }
            }
        }
    }
...
private fun VariableDeductionCard(value: String, onValueChange: (String) -> Unit, tax: Double, wop: Double, total: Double) {
    FinanceCard(accent = Orange) {
        Text("DEDUCTIONS", color = Orange, fontSize = 9.sp, fontWeight = FontWeight.Black)
        Text("Monthly commitments", color = Ink, fontSize = 18.sp, fontWeight = FontWeight.Black)
        Spacer(Modifier.height(8.dp))
        MoneyField("Other deduction", value, onValueChange)
        Spacer(Modifier.height(8.dp))
        MetricRow("APIT", formatRs(tax))
        MetricRow("WOP", formatRs(wop))
        MetricRow("Total deductions", formatRs(total))
    }
}

@Composable
private fun CompactToolCard(title: String, subtitle: String, icon: androidx.compose.ui.graphics.vector.ImageVector, accent: Color, onClick: () -> Unit) {
    Card(modifier = Modifier.width(210.dp), shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Column(modifier = Modifier.padding(16.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) { Icon(icon, contentDescription = null, tint = accent); Spacer(Modifier.width(8.dp)); Text(title, fontWeight = FontWeight.Bold, color = Ink, fontSize = 14.sp) }
            Spacer(Modifier.height(4.dp)); Text(subtitle, color = Slate, fontSize = 11.sp)
            Spacer(Modifier.height(8.dp)); Text("Open", color = accent, fontSize = 11.sp, fontWeight = FontWeight.Bold, modifier = Modifier.padding(vertical = 4.dp))
        }
    }
}
...
private fun MonthlyRecordPreview(basicSalary: Double, otHours: Double, dutyHours: Double, phHours: Double, gross: Double, net: Double, deductions: Double, workingDays: Double) {
    FinanceCard(accent = Mint) {
        Text("MONTHLY RECORD", color = Mint, fontSize = 9.sp, fontWeight = FontWeight.Black)
        Text("Current working summary", color = Ink, fontSize = 18.sp, fontWeight = FontWeight.Black)
        ...
        MetricRow("Net", formatRs(net))
    }
}

@Composable
private fun FinanceCard(accent: Color, onClick: (() -> Unit)? = null, content: @Composable () -> Unit) {
    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = Color.White), onClick = onClick ?: {}) {
        Column(modifier = Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(6.dp), content = content)
    }
}
