...
@Composable
fun FinancialDashboardScreen(financialState: AdvancedFinanceUiState, onNavigate: (String) -> Unit, onBack: () -> Unit) {
    ...
    Scaffold(topBar = {
        TopAppBar(
            title = { Column { Text("Financial Dashboard", fontWeight = FontWeight.ExtraBold, fontSize = 20.sp, color = Ink); Text("OT • Duty • Salary", fontSize = 11.sp, color = Slate, fontWeight = FontWeight.Medium) } },
            navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.Default.ArrowBack, contentDescription = "Back") } },
            colors = TopAppBarDefaults.topAppBarColors(containerColor = Color.White)
        )
    }, containerColor = FinanceBackground) { innerPadding ->
        Column(modifier = Modifier.fillMaxSize().padding(innerPadding).verticalScroll(scrollState).padding(horizontal = 16.dp, vertical = 14.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                Column(modifier = Modifier.padding(18.dp)) {
                    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column { Text("Current Earnings Trajectory", fontSize = 16.sp, fontWeight = FontWeight.Bold, color = Ink); Text("Basic + allowances + OT", fontSize = 12.sp, color = Slate) }
                        }
            ...
            Row(modifier = Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                CompactToolCard("Loan Amortization", "Plan repayments", Icons.Default.Calculate, Violet, onClick = { onNavigate("loan_aggregator") })
                CompactToolCard("Monthly Record", "Review this month", Icons.Default.CalendarMonth, Cyan, onClick = { showHistory = !showHistory })
            }
...
@Composable private fun FinanceCard(accent: Color, content: @androidx.compose.runtime.Composable androidx.compose.foundation.layout.ColumnScope.() -> Unit) { Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = Color.White), border = androidx.compose.foundation.BorderStroke(1.dp, accent.copy(alpha = 0.12f))) { Column(modifier = Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) { content() } } }
